class AdminPanel {
    ...
    initializeElements() {
        ...
        this.serviceRequestsCountEl = document.getElementById('service-requests-count');
    ...
    bindEvents() {
        ...
        if (this.statusFilter) {
            this.statusFilter.addEventListener('change', () => this.loadServiceRequests());
        }
    ...
    updateStats(stats) {
        if (this.totalUsersEl) this.totalUsersEl.textContent = stats.totalUsers || 0;
        if (this.totalMessagesEl) this.totalMessagesEl.textContent = stats.totalMessages || 0;
        if (this.serviceRequestsCountEl) this.serviceRequestsCountEl.textContent = stats.serviceRequests || 0;
        if (this.todayUsersEl) this.todayUsersEl.textContent = stats.todayUsers || 0;
    }
    ...
    updateRecentActivity(activities) {
        ...
        if (!activities || activities.length === 0) {
            activityList.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">Henüz aktivite bulunmuyor.</p>';
            return;
        }
    ...
    async loadUsers() {
        try {
            ...
            const response = await fetch(`${this.apiBaseUrl}/admin/users?filter=${filter}`);
    ...
    updateUsersTable(users) {
        ...
        if (!users || users.length === 0) {
            this.usersTableBody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px; color: #666;">Henüz kullanıcı bulunmuyor.</td></tr>';
            return;
        }
        ...
        this.usersTableBody.innerHTML = users.map(user => `
            <tr>
                ...
                <td>${user.email}</td>
                <td><span class="status-badge ${user.email_verified ? 'status-verified' : 'status-unverified'}">${user.email_verified ? 'Doğrulandı' : 'Beklemede'}</span></td>
                <td><span class="status-badge ${user.product_verified ? 'status-verified' : 'status-unverified'}">${user.product_verified ? 'Doğrulandı' : 'Beklemede'}</span></td>
                <td>${this.formatDate(user.created_at)}</td>
                <td>${user.last_activity ? this.formatDate(user.last_activity) : 'Hiç'}</td>
    ...
    async loadChats() {
        try {
            ...
            const response = await fetch(`${this.apiBaseUrl}/admin/chats?filter=${filter}&date=${date}`);
    ...
    updateChatList(chats) {
        ...
        if (!chats || chats.length === 0) {
            this.chatList.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">Henüz sohbet bulunmuyor.</p>';
            return;
        }
        ...
        this.chatList.innerHTML = chats.map(chat => `
            <div class="chat-item">
                <div class="chat-header">
                    ...
                    <div class="chat-time">${this.formatDate(chat.last_message_time)}</div>
                </div>
    ...
    updateAnalytics(analytics) {
        ...
        if (locationList && analytics.locations) {
            locationList.innerHTML = analytics.locations.map(loc => `
        ...
        if (questionStats && analytics.questions) {
            questionStats.innerHTML = analytics.questions.map(q => `
    ...
    async loadServiceRequests() {
        try {
            ...
            const response = await fetch(`${this.apiBaseUrl}/admin/service-requests?status=${status}`);
    ...
    updateServiceRequests(requests) {
        ...
        if (!requests || requests.length === 0) {
            this.serviceRequestsGrid.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">Henüz servis talebi bulunmuyor.</p>';
            return;
        }
        
        this.serviceRequestsGrid.innerHTML = requests.map(request => `
            <div class="service-request-card">
                <div class="service-header">
                    <div class="service-title">${request.name}</div>
                    <div class="service-date">${this.formatDate(request.created_at)}</div>
                </div>
                <div class="service-info">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">${request.product_name || 'Belirtilmemiş'}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Konum</div>
                        <div class="info-value">${request.location_address || 'Bilinmiyor'}</div>
                    </div>
                </div>
